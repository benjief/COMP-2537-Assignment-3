...
app.use('/favicon', express.static('assets/favicon'));
...
app.get('/profile', function(req, res) {
    ...
    if(req.session.loggedIn) {
        ...
        let templateFile = fs.readFileSync('./assets/html/profile.html', "utf8");
        ...
        let left = fs.readFileSync('./assets/html/profile-left-content.html', "utf8");
        ...
        // insert the left column from a different file (or could be a DB or ad network, etc.)
        let middle = fs.readFileSync('./assets/html/profile-middle-content.html', "utf8");
        ...
        // insert the left column from a different file (or could be a DB or ad network, etc.)
        let right = fs.readFileSync('./assets/html/profile-right-content.html', "utf8");
...
app.get('/recipe', function(req, res) {
    ...
    if(req.session.loggedIn) {
        ...
        // DIY templating with DOM, this is only the husk of the page
        let templateFile = fs.readFileSync('./assets/html/recipe.html', "utf8");
        let templateDOM = new JSDOM(templateFile);
        let $template = require("jquery")(templateDOM.window);
        ...
        // insert the left column from a different file (or could be a DB or ad network, etc.)
        let left = fs.readFileSync('./assets/html/recipe-left-content.html', "utf8");
        let leftDOM = new JSDOM(left);
        let $left = require("jquery")(leftDOM.window);
        // Replace!
        $template("#left_placeholder").replaceWith($left("#left_column"));
        ...
        // insert the left column from a different file (or could be a DB or ad network, etc.)
        let middle = fs.readFileSync('./assets/html/recipe-middle-content.html', "utf8");
        let middleDOM = new JSDOM(middle);
        let $middle = require("jquery")(middleDOM.window);
        // Replace!
        $template("#middle_placeholder").replaceWith($middle("#middle_column"));
        ...
        // insert the left column from a different file (or could be a DB or ad network, etc.)
        let right = fs.readFileSync('./assets/html/recipe-right-content.html', "utf8");
        let rightDOM = new JSDOM(right);
        let $right = require("jquery")(rightDOM.window);
        // Replace!
        $template("#right_placeholder").replaceWith($right("#right_column"));
